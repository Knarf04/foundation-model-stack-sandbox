...
class MultiHeadAttention(nn.Module):
    ...
    def _gen_affinity_scores(self, k, src, dest, r):
        ...
        affinity = torch.einsum('bnqh, bnkh -> bnqk', k*dest.sqrt().unsqueeze(-1), k*src.sqrt().unsqueeze(-1)).relu().float().pow(2)        affinity = torch.log1p(affinity.clamp(min=0, max=1-1e-6).neg())

    ...
    def forward(
        ...
        **attn_kwargs: Unpack[AttentionKwargs],
        ...
        if past_key_value_state is not None:
            ...
            # Calculate decays: kk.relu() * sqrt(src), matching prefill affinity
            ...
            if q_len <= chunk_size:
                ...
                # Math: affinity[q,k] = (keys[q] · keys[k]) * sqrt(src[k])  (since dest=ones)
                ...
                for i in range(n_chunks):
                    ...
                    for j in range(i + 1):
                        ...
                        c_k = kj - ki
                        # Pairwise log-decay using actual keys (matching _gen_affinity_scores)
                        # dest=ones so q-side is just keys, k-side is keys * sqrt(src)

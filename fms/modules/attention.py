...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        **attn_kwargs: Unpack[AttentionKwargs],
        ...
        if past_key_value_state is not None:
            ...
            # Calculate decays: kk * sqrt(dest) * sqrt(src), matching prefill affinity
            decay = kk.relu().float() * static_dest.unsqueeze(-1).sqrt() * r.sqrt()
            ...
            if q_len <= chunk_size:
                ...
                # Math: affinity[q,k] = (keys[q]·keys[k]) * sqrt(dest[q]) * sqrt(src[k])
                # Then: log1p(-relu(affinity).clamp(max=1-1e-6))
                ...
                for i in range(n_chunks):
                    ...
                    for j in range(i + 1):
                        ...
                        c_k = kj - ki
                        # Pairwise log-decay matching _gen_affinity_scores:
                        # einsum(keys * sqrt(dest), keys * sqrt(src))
                        log_decay = torch.einsum(
                            'bhqd, bhkd -> bhqk',
                            keys[:, :, qi:qj] * static_dest[:, :, qi:qj].sqrt().unsqueeze(-1),
                            keys[:, :, ki:kj] * static_src[:, :, ki:kj].sqrt().unsqueeze(-1)
                        )  # b h c_q c_k
                        log_decay = torch.log1p(
                            log_decay.relu().float().clamp(max=1-1e-6).neg()
                        )
                        if j == i:
                ...
                aux = prefix.exp().gt(.001).to(keys.dtype).mean()
    ...
    def _gen_affinity_scores(self, k, src, dest, r):
        ...
        aux = cache_affs.exp().gt(.001).to(affinity.dtype).mean()

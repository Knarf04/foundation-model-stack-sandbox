...
def _sink_scale(
    ...
    """
    Validate the sink trio and return sigmoid(lse - sinks) in fp32, [B, N, Hq].
    Forward and backward share it so the backward differentiates exactly the
    scale the forward applied.
    """
    B, N, Hq, _ = out.shape
    check_shape("lse", lse, (B, N, Hq))
    check_shape("sinks", sinks, (Hq,), "one logit per query head")
    return torch.sigmoid(lse.float() - sinks.float()[None, None, :])
...
def apply_attention_sink(
    ...
    """
    scale = _sink_scale(out, lse, sinks)
...
def attention_sink_backward(
    ...
    """
    r = _sink_scale(out, lse, sinks)                # fp32, validates lse/sinks
    check_shape("dout", dout, out.shape)
...
def apply_output_gate(
    ...
    check_shape("gate_weight", gate_weight, (Hq * Dv, x.shape[-1]))

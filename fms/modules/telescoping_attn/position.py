...
def relative_bin_distance(
    ...
    visible to q, 0 = newest. -> int in [0, M-1]; M <= cache_size. Raises if
    the entry is not visible.

    Written for an arbitrary token position, hence the clamp/ceil sum; for
    visible entries telescoping_attention.bin_distances uses the cheaper
    base[l] - (j - lo_l) form, checked against this.
    """
...
def _tile_bin_distances(
    ...
    -> dist [M, Kt] long. Valid lanes must satisfy 0 <= dist <
    max_relative_bins or this raises -- a negative index would alias the
    tensor end. Invalid lanes may go negative.
    """
    M = row_lo.shape[0]
    ...
    where = f"query block [{q_start},{q_end}) at level {level}"
    if bool((valid & (dist >= max_relative_bins)).any()):
        raise ValueError(
            f"relative bin distance {int(dist[valid].max())} in {where} "
            f"exceeds max_relative_bins={max_relative_bins}; no silent "
            f"clamping -- a table with >= cache_size={spec.cache_size} bins "
            f"always suffices"
        )
    if bool((valid & (dist < 0)).any()):
        raise ValueError(
            f"negative relative bin distance {int(dist[valid].min())} on a "
            f"valid lane in {where}; range invariants guarantee 0 <= dist for "
            f"visible entries, so this input is semantically broken"
        )
    return dist
